...
Test(command_parse, ls_command) {
	...
	cr_assert_str_eq(cmd->argv[0], "ls");
	cr_assert_eq(cmd->argc, 1);
	cr_assert_str_eq(cmd->argv[0], cmd->argv[0]);
	cr_assert_null(cmd->argv[1]);
}
...
Test(command_parse, git_multiple_arguments_without_quotes) {
	...
	cr_assert_str_eq(cmd->argv[0], "git");
	cr_assert_eq(cmd->argc, 4);
	cr_assert_str_eq(cmd->argv[0], cmd->argv[0]);
	cr_assert_str_eq(cmd->argv[1], "commit");
	cr_assert_str_eq(cmd->argv[2], "-m");
	cr_assert_str_eq(cmd->argv[3], "Message");
}
...
Test(command_parse, git_multiple_arguments_with_quotes_and_spaces) {
	...
	cr_assert_str_eq(cmd->argv[1], "commit");
	cr_assert_str_eq(cmd->argv[2], "-m");
	cr_assert_str_eq(cmd->argv[3], "Message with spaces");
}

Test(command_parse, echo_valid) {
	command* cmd = command_parse("echo test'test'");

	cr_assert_str_eq(cmd->argv[0], "echo");
	cr_assert_eq(cmd->argc, 2);
	cr_assert_str_eq(cmd->argv[1], "testtest");
}
...
Test(command_parse, echo_valid_with_quotes) {
	...
	cr_assert_str_eq(cmd->argv[0], "echo");
	cr_assert_eq(cmd->argc, 2);
	cr_assert_str_eq(cmd->argv[1], "out in out");
}
...
Test(command_parse, echo_valid_with_double_quotes) {
	...
	cr_assert_str_eq(cmd->argv[0], "echo");
	cr_assert_eq(cmd->argc, 2);
	cr_assert_str_eq(cmd->argv[1], "out in out in ");
}
...
Test(command_parse, simple_echo_command) {
	...
	cr_assert_str_eq(cmd->argv[1], "Testing");
	cr_assert_str_eq(cmd->argv[2], "Test");
}
...
Test(command_parse, simple_echo_pipe_command_chain) {
	...
	cr_assert_str_eq(cmd->next->argv[0], "echo");
	cr_assert_eq(cmd->next->argc, 2);
	cr_assert_str_eq(cmd->next->argv[1], "123123");
}
...
Test(command_parse, and_command_chain) {
	...
	cr_assert_eq(cmd->argc, 3);
	cr_assert_str_eq(cmd->argv[1], "test");
	cr_assert_str_eq(cmd->argv[2], "test1");
	cr_assert_eq(cmd->chain_type, AND);

	cr_assert_str_eq(cmd->next->argv[0], "cat");
	cr_assert_eq(cmd->next->argc, 2);
	cr_assert_str_eq(cmd->next->argv[1], "123123");
}
...
Test(command_parse, command_chain_with_escaping_character) {
	...
	cr_assert_str_eq(cmd->argv[0], "cat");
	cr_assert_eq(cmd->argc, 2);
	cr_assert_str_eq(cmd->argv[1], "test test1");
...
Test(command_parse, command_stdout_redirection) {
	...
	cr_assert_str_eq(cmd->argv[0], "echo");
	cr_assert_eq(cmd->argc, 2);
	cr_assert_str_eq(cmd->argv[1], "123123");
...
Test(command_parse, complex_command) {
	...
	cr_assert_eq(cmd->argc, 2);
	cr_assert_str_eq(cmd->argv[0], cmd->argv[0]);
	cr_assert_str_eq(cmd->argv[1], "123123 1");
